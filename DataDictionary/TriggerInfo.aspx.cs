...
{
    public partial class TriggerInfo : System.Web.UI.Page
    {
        ...
        {
            if (Request.QueryString["triname"] != null)
            {
                BindProcInfo(Request.QueryString["triname"].ToString());
            }
        }
        private void BindProcInfo(string triname)
        {
            DataSet ds = new DataSet();
            connStr = Session["connStr"].ToString();
            ViewState["triname"] = triname;
            query = @"
...
WHERE row = 1 and proc_name in('" + triname + "') ORDER BY proc_name,table_name ";
